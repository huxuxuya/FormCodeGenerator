...
Процедура СформироватьКодПоСтрокам(ДеревоЭлементов, ПараметрыФорм)
	...
	Для Индекс = 1 По КоличествоСтрок Цикл
		...
		ФильтроватьМетодыДобавленияПолейФормы(МетодыТипов, ТекЭлемент, ТипРодителяСтрокой);
		...
		Если МетодыТипов.Свойство(ТипСтрокой) Тогда
			...
			ИначеЕсли ТипСтрокой = "ПолеФормы" Тогда
				СтрокаЭлемента.ТекстНачало = ТекстНовыйКонтекст;
				СтрокаЭлемента.ТекстСоздания = СтрШаблон(ШаблонПоле, ИмяМетода, ИмяЭлемента);				
			КонецЕсли;
		КонецЕсли;	
...
Процедура ФильтроватьМетодыДобавленияПолейФормы(МетодыТипов, ТекЭлемент, ТипРодителяСтрокой)
