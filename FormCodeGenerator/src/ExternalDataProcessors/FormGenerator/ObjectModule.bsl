...
Процедура СформироватьКодПоСтрокам(ДеревоЭлементов, ПараметрыФорм)
	...
	Для Индекс = 1 По КоличествоСтрок Цикл
		...
		Если ТипСтрокой = "ГруппаФормы" Тогда
			...
			//Чем больше число приоритета, тем он выше
			...
			СтруктураКопируемыхСвойств = СтруктураДанныхДефолтногоЭлемента.СтруктураКопируемыхСвойств;			
			...
			Если ТаблицаРасчетаПоинтов.Количество() > 0 Тогда
				ИмяМетода = ТаблицаРасчетаПоинтов[0].Метод;
			КонецЕсли;
			...
			ТекстНовыйКонтекст = "Контекст = РедакторФорм.НовыйКонтекстЭлемента(ЭтотОбъект);";
 			...
 			ШаблонГруппа = "НовыйЭлемент = РедакторФорм.%1(КонтекстСтраницы, ""%2"");";
 			
 			ТекстГруппа = СтрШаблон(ШаблонГруппа, ИмяМетода, ИмяЭлемента);
