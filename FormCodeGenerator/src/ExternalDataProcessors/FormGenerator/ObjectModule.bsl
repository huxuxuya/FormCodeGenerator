...
Процедура СформироватьКодПоСтрокам(ДеревоЭлементов, ПараметрыФорм)
	...
	Для Индекс = 1 По КоличествоСтрок Цикл
		...
		ТекстСозданияЭлемента = НСтр("ru = 'Не поддерживается'"); 
						
		ШаблонКонтекстБезРодителя = НСтр("ru = 'Контекст = РедакторФорм.НовыйКонтекстЭлемента(ЭтотОбъект);'");
		ШаблонКонтекст =  НСтр("ru = 'Контекст = РедакторФорм.НовыйКонтекстЭлемента(ЭтотОбъект, %1);'");
		ШаблонДекорация = НСтр("ru = 'НовыйЭлемент = РедакторФорм.%1(Контекст, ""%2"", %3, %4);'");	
		ШаблонКнопка =  НСтр("ru = 'НовыйЭлемент = РедакторФорм.%1(Контекст, ""%2"", %3, ""%4"")'");	
		ШаблонГруппа =  НСтр("ru = 'НовыйЭлемент = РедакторФорм.%1(Контекст, ""%2"");'");
		ШаблонПоле =  НСтр("ru = 'НовыйЭлемент = РедакторФорм.%1(Контекст, ""%2"");'");

		
		Если СтрокаЭлемента.Родитель = Неопределено Тогда
			ТекстНовыйКонтекст = ШаблонКонтекстБезРодителя
		Иначе
			ТекстНовыйКонтекст = СтрШаблон(ШаблонКонтекст, Родитель);
		КонецЕсли;
