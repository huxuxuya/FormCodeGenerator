...
Процедура ПодготовитьДанныеАнализируемойФормы(Форма) Экспорт
	...
	Если Форма.Параметры.Свойство("РедакторФорм") Тогда
		...
		ПараметрыФорм.Вставить("ФормаОбразец", Форма);
		ПараметрыФорм.Вставить("ИмяМодуля", Форма.Параметры.РедакторФорм.ИмяМодуля);
...
Процедура СформироватьКодПоСтрокам(ДеревоЭлементов, ПараметрыФорм, КешСоответствияПредставленийТипов = Неопределено)
	...
	Для Индекс = 1 По КоличествоСтрок Цикл
		...
		Шаблоны = ШаблоныМодуляГенерацииКода(ИмяМодуля, ИмяЭлемента, ТипЭлемента, Родитель, СледующийЭлемент);
				
		ТекстНовыйКонтекст = ТекстНовогоКонтекста(Шаблоны, СтрокаЭлемента, Родитель, ТекстНовыйКонтекст);
								
		СтрокаЭлемента.ТекстСоздания = Шаблоны.ТекстСозданияЭлемента;
		...
		Если МетодыТипов.Свойство(ТипСтрокой) Тогда
			...
			Если ТипСтрокой = "ГруппаФормы" Тогда
				...
				СтрокаЭлемента.ТекстСоздания = СтрШаблон(Шаблоны.ШаблонГруппа, ИмяМетода, ИмяЭлемента);
			...
			ИначеЕсли ТипСтрокой = "ДекорацияФормы" Тогда 
				...
				ТекстСоздания = СтрШаблон(Шаблоны.ШаблонДекорация, ИмяМетода, ИмяЭлемента, Заголовок, РастягиватьПоГоризонтали);
			...
			ИначеЕсли ТипСтрокой = "ПолеФормы" Тогда
				...
				СтрокаЭлемента.ТекстСоздания = СтрШаблон(Шаблоны.ШаблонПоле, ИмяМетода, ИмяРеквизита);	
				// ПутьКДанным для полей формы является параметром вызываемого метода.
				Если ЕстьСвойствоКоллекции(ФормаОбразец, "Объект") Тогда
			...
			ИначеЕсли ТипСтрокой = "КнопкаФормы" Тогда 			
				...
				СтрокаЭлемента.ТекстСоздания = СтрШаблон(Шаблоны.ШаблонКнопка, ИмяКнопки, Заголовок);	
							
				ШабонОбслуживающегоМетода = НСтр("ru = '&НаКлиенте 
												 |Процедура Подключаемый_%1(Команда)
												 |
												 |КонецПроцедуры
												 |'");	
							
				СтрокаЭлемента.ТекстОбслуживающихПроцедур = СтрШаблон(ШабонОбслуживающегоМетода, ИмяКнопки);
			
			ИначеЕсли ТипСтрокой = "ТаблицаФормы" Тогда 
				
				// TODO: Добавить поддержку таблицы формы.
...
Функция ИмяМетодаРассчитнноеПоВесу(СтруктураДанныхДефолтногоЭлемента, МетодыДобавленияЭлементов, ТекЭлемент, ИмяМетода, КешСоответствияПредставленийТипов)
	...
	Если МетодыДобавленияЭлементов.Свойство(ТипЭлементаСтрокой) Тогда

		ОбслуживающиеМетоды = МетодыДобавленияЭлементов[ТипЭлементаСтрокой];
...
#Область ГруппаФормы
	...
	СвойстваПоУмолчанию = Новый Структура;
		...
		СвойстваПоУмолчанию.Вставить("Вид", Неопределено);
...
Процедура ФильтроватьМетодыДобавленияПолейФормы(МетодыТипов, ЭтоПолеРеквизитОбъекта)
	...
	Иначе
		СтруктураМетодовПолеФормы.Удалить("НовоеМногострочноеПолеРеквизитОбъекта");
		СтруктураМетодовПолеФормы.Удалить("НовоеПолеФлажокФормыРеквизитОбъекта");
		СтруктураМетодовПолеФормы.Удалить("НовоеПолеШапкиФормыРеквизитОбъекта");
...
Функция ТекстНовогоКонтекста(Шаблоны, СтрокаЭлемента, Родитель, ТекстНовыйКонтекст)
	
	Если СтрокаЭлемента.Родитель = Неопределено Тогда
		ТекстНовыйКонтекст = Шаблоны.ШаблонКонтекстБезРодителя
	Иначе
		ТекстНовыйКонтекст = СтрШаблон(Шаблоны.ШаблонКонтекст, Родитель);
	КонецЕсли;
	
	Возврат ТекстНовыйКонтекст;	
КонецФункции


Функция ШаблоныМодуляГенерацииКода(ИмяМодуля, ИмяЭлемента, ТипЭлемента, Родитель, СледующийЭлемент)
	
	СтруктураШаблонов = Новый Структура;
		
		
	ТекстСозданияЭлемента = НСтр("ru = 'НовыйЭлемент = Этаформа.Элементы.Вставить(""%1, %2, %3, %4);'"); 
	СтруктураШаблонов.Вставить("ТекстСозданияЭлемента", СтрШаблон(ТекстСозданияЭлемента, ИмяЭлемента, ТипЭлемента, Родитель, СледующийЭлемент));
					
	ШаблонКонтекстБезРодителя = НСтр("ru = 'Контекст = %1.НовыйКонтекстЭлемента(ЭтотОбъект);'");
	СтруктураШаблонов.Вставить("ШаблонКонтекстБезРодителя", СтрШаблон(ШаблонКонтекстБезРодителя, ИмяМодуля));
	
	ШаблонКонтекст =  НСтр("ru = 'Контекст = %1.НовыйКонтекстЭлемента(ЭтотОбъект, %2);'");
	СтруктураШаблонов.Вставить("ШаблонКонтекст", СтрШаблон(ШаблонКонтекст, ИмяМодуля, "%1"));
	
	ШаблонДекорация = НСтр("ru = 'НовыйЭлемент = %1.%2(Контекст, ""%3"", %4, %5);'");	
	СтруктураШаблонов.Вставить("ШаблонДекорация", СтрШаблон(ШаблонДекорация, ИмяМодуля, "%1", "%2", "%3","%4"));
	
	ШаблонКнопка = НСтр("ru = 'НовыйЭлемент = %1.НоваяКомандаИКнопкаФормы(Контекст, ""%2"", ""%3"");'"); 	
	СтруктураШаблонов.Вставить("ШаблонКнопка", СтрШаблон(ШаблонКнопка, ИмяМодуля, "%1", "%2"));
			
	ШаблонГруппа =  НСтр("ru = 'НовыйЭлемент = %1.%2(Контекст, ""%3"");'");
	СтруктураШаблонов.Вставить("ШаблонГруппа", СтрШаблон(ШаблонГруппа, ИмяМодуля, "%1", "%2"));
			
	ШаблонПоле =  НСтр("ru = 'НовыйЭлемент = %1.%2(Контекст, ""%3"");'");
	СтруктураШаблонов.Вставить("ШаблонПоле", СтрШаблон(ШаблонПоле, ИмяМодуля, "%1", "%2"));
...
Процедура УстановитьОтличающиесяСвойства(СтрокаЭлемента, СтруктураДанныхЭлемента, ТекЭлемент, ПараметрыФорм)
	...
	ШаблонОбработкаВыбора = Нстр("ru = '
                                  |&НаКлиенте
                                  |Процедура %1(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
                                  |	
                                  |КонецПроцедуры
                                  |'");
	
	ШаблонОдинЭлементВПараметре = Нстр("ru = '
                               |&НаКлиенте
                               |Процедура %1(Элемент)
                               |	
                               |КонецПроцедуры
	...
	ШаблонЭлементИОтказ = Нстр("ru = '
                               |&НаКлиенте
                               |Процедура %1(Элемент, Отказ)
                               |	
                               |КонецПроцедуры
                               |'");
	
	
	ШаблонНачалоВыбора = Нстр("ru = '&НаКлиенте
                               |Процедура %1(Элемент, ДанныеВыбора, СтандартнаяОбработка)
                               |	
                               |КонецПроцедуры
                               |'");
                               
	ШаблонВыбор = Нстр("ru = '&НаКлиенте
					   |Процедура %1(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
					   |	
					   |КонецПроцедуры
					   |'");                               
	
	ШаблонЭлементСтандартнаяОбработка = Нстр("ru = '&НаКлиенте
                           |Процедура %1(Элемент, СтандартнаяОбработка)
                           |	
                           |КонецПроцедуры'");

	ШаблонПеретаскиваниеИПроверкаПеретаскивания = Нстр("ru = '&НаКлиенте
                                         |Процедура %1(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
                                         |	
                                         |КонецПроцедуры
                                         |'");

	ШаблонПриОкончанииРедактирования = Нстр("ru = '&НаКлиенте
                                             |Процедура %1(Элемент, НоваяСтрока, ОтменаРедактирования)
                                             |	
                                             |КонецПроцедуры'");
	ШаблонРегулирование = Нстр("ru = '&НаКлиенте
                                |Процедура %1(Элемент, Направление, СтандартнаяОбработка)
                                |
                                |КонецПроцедуры
                                |'");
                                
	ШаблонАвтоПодбор = Нстр("ru = '&НаКлиенте
                             |Процедура %1(Элемент, Текст, ДанныеВыбора, Ожидание, СтандартнаяОбработка)
                             |
                             |КонецПроцедуры
                             |'");

	ШаблонОкончаниеВводаТекста = Нстр("ru = '	
                             |&НаКлиенте
                             |Процедура %1(Элемент, Текст, ДанныеВыбора, СтандартнаяОбработка)
                             |
                             |КонецПроцедуры
                             |'");
	

	ШаблонПриНачалеРедактирования = Нстр("ru = '	
                             |&НаКлиенте
                             |Процедура %1(Элемент, НоваяСтрока, Копирование)
                             |
                             |КонецПроцедуры
                             |'");
                             	
	СтруктураШаблонов = Новый Структура;
	СтруктураШаблонов.Вставить("ОбработкаВыбора", ШаблонОбработкаВыбора);
	СтруктураШаблонов.Вставить("Выбор", ШаблонВыбор);
	СтруктураШаблонов.Вставить("ПриИзменении", ШаблонОдинЭлементВПараметре);
	СтруктураШаблонов.Вставить("НачалоВыбора", ШаблонНачалоВыбора );
	СтруктураШаблонов.Вставить("Нажатие", ШаблонОдинЭлементВПараметре);
	СтруктураШаблонов.Вставить("Очистка", ШаблонЭлементСтандартнаяОбработка);
	СтруктураШаблонов.Вставить("ПроверкаПеретаскивания", ШаблонПеретаскиваниеИПроверкаПеретаскивания );
	СтруктураШаблонов.Вставить("Перетаскивание", ШаблонПеретаскиваниеИПроверкаПеретаскивания );
	СтруктураШаблонов.Вставить("ПриАктивизацииСтроки", ШаблонОдинЭлементВПараметре);
	СтруктураШаблонов.Вставить("ПриАктивизацииЯчейки", ШаблонОдинЭлементВПараметре );	
	СтруктураШаблонов.Вставить("ПередУдалением", ШаблонЭлементИОтказ);
	СтруктураШаблонов.Вставить("ПослеУдаления", ШаблонОдинЭлементВПараметре);	
	СтруктураШаблонов.Вставить("ПриНачалеРедактирования", ШаблонПриНачалеРедактирования );
	СтруктураШаблонов.Вставить("ПриОкончанииРедактирования", ШаблонПриОкончанииРедактирования );
	СтруктураШаблонов.Вставить("Регулирование", ШаблонРегулирование);
	СтруктураШаблонов.Вставить("АвтоПодбор", ШаблонАвтоПодбор);
	СтруктураШаблонов.Вставить("ОкончаниеВводаТекста", ШаблонОкончаниеВводаТекста);
	СтруктураШаблонов.Вставить("Открытие", ШаблонЭлементСтандартнаяОбработка);
	...
	Для каждого Свойство Из СтруктураОбработчиков Цикл
		...
		Если НЕ ПустаяСтрока(ОбработчикИмя) Тогда
			...
			ТекстПослеСоздания = СтрокаЭлемента.ТекстПослеСоздания;
			
			ТекстПослеСоздания = ТекстПослеСоздания + Символы.ПС+
			"НовыйЭлемент.УстановитьДействие("""+Свойство.Ключ+""","""+ОбработчикИмя+""");";
			
			СтрокаЭлемента.ТекстПослеСоздания = ТекстПослеСоздания;
			
			ДобавитьТекст(СтрокаЭлемента.ТекстОбслуживающихПроцедур ,СтрШаблон(СтруктураШаблонов[Свойство.Ключ], ОбработчикИмя));
...
Функция ДопустимыеТипы()
	...
	Для каждого Документ Из Метаданные.Документы Цикл
		...
		МассивТипов.Добавить("ДокументОбъект."+Документ.Имя);	
	КонецЦикла; 
	Для каждого Справочник Из Метаданные.Справочники Цикл
		МассивТипов.Добавить("СправочникСсылка."+Справочник.Имя);
		МассивТипов.Добавить("СправочникОбъект."+Справочник.Имя);	
	КонецЦикла; 
	Для каждого Справочник Из Метаданные.РегистрыСведений Цикл
		МассивТипов.Добавить("РегистрСведенийМенеджерЗаписи."+Справочник.Имя);	
		МассивТипов.Добавить("РегистрСведенийНаборЗаписей."+Справочник.Имя);	
	КонецЦикла; 
...
Функция ПолучитьТекстовоеПредставлениеСложногоТипа(Значение)
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("Цвет") Тогда
		...
		КонецЕсли;	
	Иначе
		// Все типы должны быть поддерживаемыми. Функцию вызвали для неподдерживаемого типа.
		Сообщить("ПолучитьТекстовоеПредставлениеСложногоТипа:"+ТипЗнч(Значение))
	КонецЕсли; 
