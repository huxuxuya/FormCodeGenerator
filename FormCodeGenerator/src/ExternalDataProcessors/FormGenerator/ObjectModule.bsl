...
Процедура СформироватьКодПоСтрокам(ДеревоЭлементов, ПараметрыФорм)
	...
	Для Индекс = 1 По КоличествоСтрок Цикл
		...
		Если МетодыТипов.Свойство(ТипСтрокой) Тогда
			...
			ИмяМетода = ИмяМетодаРассчитнноеПоВесу(СтруктураДанныхДефолтногоЭлемента, ТекЭлемент, ИмяМетода);
...
Функция ИмяМетодаРассчитнноеПоВесу(СтруктураДанныхДефолтногоЭлемента, ТекЭлемент, ИмяМетода)

	ТипЭлементаСтрокой = ПолучитьСтроковоеПредставлениеТипа(ТипЗнч(ТекЭлемент));
	МетодыДобавленияЭлементов = МетодыДобавленияЭлементовНаФорму();
