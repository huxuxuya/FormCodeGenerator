...
Процедура СформироватьКодПоСтрокам(ДеревоЭлементов, ПараметрыФорм)
	...
	Для Индекс = 1 По КоличествоСтрок Цикл
		...
		Если ТипСтрокой = "ГруппаФормы" Тогда
			...
			// Приоритет - чем меньше, тем выше приоритет. При равных весах, первым выбирается метод наименьшего приоритета.
			...
			СвойстваПоУмолчанию = Новый Структура;
			СвойстваПоУмолчанию.Вставить("Приоритет", 1);	
			СвойстваПоУмолчанию.Вставить("Вид", "ВидГруппыФормы.ОбычнаяГруппа");
			...
			//Переопределить дефолтные свойства элемента, дефолтными свойствами метода
						
			
			ТекстНовыйКонтекст = "Контекст = РедакторФорм.НовыйКонтекстЭлемента(ЭтотОбъект);";
 			ШаблонСвойства = "Контекст.Свойства.Вставить(""%1"", %2);";
 			ШаблонГруппа = "НовыйЭлемент = РедакторФорм.НоваяГруппаФормы(КонтекстСтраницы, ""%1"");";
 			
 			ТекстГруппа = СтрШаблон(ШаблонГруппа, ИмяЭлемента);
 			
 			СтрокаЭлемента.ТекстНачало = ТекстНовыйКонтекст;
 			
 			ТекстСозданияЭлемента = ТекстГруппа; 
			
		КонецЕсли;
		
		ЗаполнитьСвойстваЭлемента(ПараметрыФорм, ТекЭлемент, СтрокаЭлемента);
...
Процедура ЗаполнитьСвойстваЭлемента(ПараметрыФорм, ТекЭлемент, СтрокаЭлемента);
	
	СтруктураДанныхЭлемента = ПолучитьСвойстваЭлемента(ТекЭлемент);
	
	УстановитьОтличающиесяСвойства(СтрокаЭлемента, СтруктураДанныхЭлемента, ТекЭлемент, ПараметрыФорм);
